...
# Predict class labels from Decision Tree
dt_preds_train = dt.predict(X_train_scaled)
dt_preds_test = dt.predict(X_test_scaled)

# Train RuleFit on DT predictions
rf = RuleFit(random_state=42)
rf.fit(X_train_scaled, dt_preds_train, feature_names=features)
...
# Output lines
output_lines = ["Top 5 RuleFit Rules (All Classes Combined) - Trained on Decision Tree\n"]
...
# Save to file
output_file = "/home/gummadi/spring25/extras/prec_cov_rulefit/mems/mems_rules_prec_cov_rulefit_dt.txt"
with open(output_file, 'a') as f:
    for line in output_lines:
        f.write(line + "\n")

print(f"✅ Decision Tree-based RuleFit summary saved to {output_file}")
